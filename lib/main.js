  ...
  provideLinter() {
    return {
      ...
      lint: async (textEditor) => {
        ...
        if (!(/.github(?:\/|\\)workflows$/.exec(dir)) || !(/jobs:/.exec(content))) return [];
        ...
        // execute the linting
        return this.helpers.exec(atom.config.get('linter-github-actions.actionsExecutablePath'), args, { stdin: content, ignoreExitCode: true }).then(output => {
          const toReturn = [];
